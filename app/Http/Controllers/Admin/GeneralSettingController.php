...
class GeneralSettingController extends Controller
{
    ...
    public function banner()
    {
        ...
        return view('admin.setting.banner', compact('page_title', 'banners'));
    }
    ...
    public function createBanner()
    {
        ...
        return view('admin.setting.create_banner', compact('page_title'));
    }
    ...
    public function editBanner($id)
    {
        ...
        return view('admin.setting.create_banner', compact('page_title', 'bannerDetail'));
    }
    ...
    public function updateBanner(Request $request)
    {
        ...
        $method = Banner::findOrFail($request->id);
        $method->title =  $request->title;
        if ($filename) {
            $method->banner_url = $filename;
        }
    ...
    public function logoIconUpdate(Request $request)
    {
        $request->validate([
            ...
            'logo_text' => 'image|mimes:jpg,jpeg,png,svg',
        ]);
        ...
        if ($request->hasFile('logo_text')) {
            try {
                $path = imagePath()['logoText']['path'];
                if (!file_exists($path)) {
                    mkdir($path, 0755, true);
                }
                Image::make($request->logo_text)->save($path . '/logoText.png');
            } catch (\Exception $exp) {
                $notify[] = ['error', 'Logo could not be uploaded.'];
                return back()->withNotify($notify);
            }    
