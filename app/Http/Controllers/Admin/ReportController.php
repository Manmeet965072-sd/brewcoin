...
class ReportController extends Controller
{
    ...
    public function wallet()
    {
        ...
        $page = 'wallet';
        //$wallet_trx = WalletsTransactions::latest()->paginate(getPaginate());
        $deposits = Deposit::with('gateway')->get();

        $withdrawals = Withdrawal::with('method')->get();

        $logs = $deposits->merge($withdrawals)->sortBy('created_at')->values()->all();
        
        $wallet_trx  = $this->paginate($logs);
        $empty_message = 'No transactions.';
    ...
    public function wallet_search(Request $request)
    {
        ...
        $wallet_trx = Deposit::with('user')->whereHas('user', function ($user) use ($search) {
    ...
    public function wallet_deposit()
    {
        ...
        $page = 'wallet.deposit';
        $deposits = Deposit::with('gateway');

        $wallet_trx = $deposits->orderBy('created_at', 'desc')->paginate(getPaginate());
    ...
    public function wallet_withdraw()
    {
        ...
        $wallet_trx =  Withdrawal::orderBy('created_at', 'desc')->paginate(getPaginate());
    ...
    public function wallet_withdraw_search(Request $request)
    {
        ...
        $wallet_trx =  Withdrawal::with('user')->whereHas('user', function ($user) use ($search) {
            $user->where('firstname', 'like', "%$search%")
                ->orWhere('lastname', 'like', "%$search%");
        })->orWhere('trx', $search)->orderBy('id', 'desc')->paginate(getPaginate());
        return view('admin.reports.wallet', compact('page_title', 'wallet_trx', 'empty_message', 'page'));
    ...
    public function wallet_transfer_trading()
    {
        ...
        $wallet_trx = WalletsTransactions::where('type', 3)->latest()->paginate(getPaginate());
        $empty_message = 'No transactions.';
        return view('admin.reports.wallet', compact('page_title', 'wallet_trx', 'empty_message', 'page'));
    }
    ...
    public function wallet_transfer_funding()
    {
        ...
        $wallet_trx = WalletsTransactions::where('type', 4)->latest()->paginate(getPaginate());
        $empty_message = 'No transactions.';
        return view('admin.reports.wallet', compact('page_title', 'wallet_trx', 'empty_message', 'page'));
    }
