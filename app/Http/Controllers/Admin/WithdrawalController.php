...
class WithdrawalController extends Controller
{
    ...
    public function details($id)
    {
        ...
        $bank_account_details = BankAccount::where('id', $withdrawal->bank_account_id)->first();
        return view('admin.withdraw.detail', compact('page_title', 'withdrawal', 'details', 'bank_account_details'));
    }
    ...
    public function approve(Request $request)
    {
        ...
        if ($request->hasFile('file')) {
            $withdraw->admin_feedback_file = uploadFile($request->file('file'), "images/withdrawals-approve");
        }
        ...
        try {
            ...
            $notificationData->fill(['user_id' => $withdraw->user_id, 'title' => "Withdrawal Status", 'message' => 'Your withdraw request for ' . round($withdraw->amount,3) .' '. $withdraw->currency . ' has been approved by admin!', 'type' => 'withdraw']);
            $notificationData->save();


            $notification = sendFCM([$user->device_token], [
                'title' => "Withdrawal Status", 'body' => 'Your withdraw request for ' . round($withdraw->amount,3) .' ' . $withdraw->currency . ' has been approved by admin!', 'icon' => asset('/assets/images/app_icon.png')
            ], [
                'type' => 'withdraw',
                'title' => "Withdrawal Status", 'notification_message' => 'Your withdraw request for ' . round($withdraw->amount,3) .' '. $withdraw->currency . ' has been approved by admin!'
            ]);
    ...
    public function reject(Request $request)
    {
      ...
      //  $wallet->save();
        ...
        try {
            $notificationData = new UserAppNotification();
            $notificationData->fill(['user_id' => $withdraw->user_id, 'title' => "Withdrawal Status", 'message' => 'Your withdraw request for ' . round($withdraw->amount,3) . ' '.$withdraw->currency . ' has been rejected by admin!', 'type' => 'withdraw']);
            $notificationData->save();


            $notification = sendFCM([$user->device_token], [
                'title' => "Withdrawal Status", 'body' => 'Your withdraw request for ' . round($withdraw->amount,3) .' '. $withdraw->currency . ' has been rejected by admin!', 'icon' => asset('/assets/images/app_icon.png')
            ], [
                'type' => 'withdraw',
                'title' => "Withdrawal Status", 'notification_message' => 'Your withdraw request for ' . round($withdraw->amount,3) . ' '.$withdraw->currency . ' has been rejected by admin!'
            ]);
