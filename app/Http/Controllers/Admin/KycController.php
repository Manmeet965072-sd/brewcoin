...
class KycController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($kyc) {
            ...
            $all_kycs = KYC::where('userId', $kyc->user->id)->get();
            $count = KYC::where('userId', $kyc->user->id)->count();
            $i = 0;
            foreach ($all_kycs as $all_kyc) {
                if ($all_kyc->status == 'approved') {
                    //$kyc->user->save();
                    $i++;
                }
            }
            ...
            if ($kyc->user) {
                try {
                    if ($i == $count && $kyc->status == 'approved' || $kyc->status == 'rejected') {
                        $notificationData = new UserAppNotification();
                        $notificationData->fill(['user_id' => $kyc->user->id, 'title' => "Kyc Status Notification", 'message' => 'Your kyc application has been ' . $kyc->status . ' by admin!', 'type' => 'kyc']);
                        ...
                        $notification = sendFCM([$kyc->user->device_token], [
                            'title' => "Kyc Status notification", 'body' => 'Your kyc application has been ' . $kyc->status . ' by admin!', 'icon' => asset('/assets/images/app_icon.png')
                        ], [
                            'type' => 'kyc', 'status' => $status,
                            'title' => "Kyc Status notification", 'notification_message' => 'Your kyc application has been ' . $kyc->status . ' by admin!'
                        ]);
