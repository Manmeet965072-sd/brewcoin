...
<div class="row" id="table-hover-row">
    ...
    <div class="col-xl-4 col-sm-6 mb-30">
        <div class="widget-two shadow rounded p-2 mb-1  bg-success">
            <div class="widget-two__content">
                <h2 class="text-white mt-1 mb-2">{{ __($general->cur_sym) }}{{ $withdrawals->where('status',1)->sum('amount') }}</h2>
                <p class="text-white">@lang('Approved Withdrawals')</p>
            </div>
        </div><!-- widget-two end -->
    </div>
    <div class="col-xl-4 col-sm-6 mb-30">
        <div class="widget-two shadow rounded p-2 mb-1 bg-6">
            <div class="widget-two__content">
                <h2 class="text-white mt-1 mb-2">{{ __($general->cur_sym) }}{{ $withdrawals->where('status',2)->sum('amount') }}</h2>
                <p class="text-white">@lang('Pending Withdrawals')</p>
            </div>
        </div><!-- widget-two end -->
    </div>
    <div class="col-xl-4 col-sm-6 mb-30">
        <div class="widget-two shadow rounded p-2 mb-1 bg-pink">
            <div class="widget-two__content">
                <h2 class="text-white mt-1 mb-2">{{ __($general->cur_sym) }}{{ $withdrawals->where('status',3)->sum('amount') }}</h2>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td data-label="{{ __('locale.Username')}}">
                                <a href="{{ route('admin.users.detail', $withdraw->user_id) }}">{{ __(@$withdraw->user->firstname).' '.__(@$withdraw->user->lastname)}}</a>
                            </td>
                            @endif
                            ...
                            <td data-label="{{ __('locale.Action')}}">
                                <a href="{{ route('admin.withdraw.details', $withdraw->id) }}" class="btn btn-icon btn-info  btn-sm " data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('locale.Detail')}}">
                                    <i class="bi bi-eye"></i>
                                </a>
                            </td>
                            ...
                            <td data-label="@lang('Info')">
                                <a href="{{ route('admin.withdraw.details', $withdraw->id) }}" class="btn btn-icon btn-info ms-1 " data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('locale.Detail')}}">
...
<div class="row">
    <div class="col">
        <form action="{{ route('admin.withdraw.search', $scope ?? str_replace('admin.withdraw.', '', request()->route()->getName())) }}" method="GET" class=" float-sm-right bg-white">
