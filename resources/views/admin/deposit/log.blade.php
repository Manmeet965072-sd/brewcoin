                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            @if(!request()->routeIs('admin.users.deposits') && !request()->routeIs('admin.users.deposits.method'))
                            @if($deposit->user)
                            <td data-label="{{ __('locale.Username')}}"><a href="{{ route('admin.users.detail', $deposit->user_id) }}">{{ $deposit->user->firstname.' '.$deposit->user->lastname }}</a></td>
                            @else
                            <td></td>
                            @endif
                            @endif
                            ...
                            <td data-label="{{ __('locale.Action')}}">
                                <a href="{{ route('admin.deposit.details', $deposit->id) }}" class="btn-icon ms-1 " data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('locale.Detail')}}">
                                    <i class="bi bi-eye"></i>
                                </a>
                            </td>
...
<div class="row">
    <div class="col"></div>
    <div class="col">
        <form action="{{route('admin.deposit.dateSearch',$scope ?? str_replace('admin.deposit.', '', request()->route()->getName()))}}" method="GET" class="bg-white">
            <div class="input-group">
                <input name="date" type="text" data-range="true" data-multiple-dates-separator=" - " data-language="en" class="datepicker-here form-control" data-position='bottom right' placeholder="{{ __('locale.Min Date - Max date')}}" autocomplete="off" value="{{ @$dateSearch }}">
