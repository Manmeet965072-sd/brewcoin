                    ...
                    <div class="row mt-2">
                        ...
                        <div class="col-xl-3 col-md-6">
                            <div class="col ">
                                ...
                                <select id="country" name="country" placeholder="Country" aria-describedby="country" value="{{ old('country') }}" class="form-control"> @include('partials.country')
                                </select>
                            </div>
                        </div>
                        <div class="col-xl-3 col-md-6">
                            ...
                            <div class="button-switch">
                                <input type="checkbox" name="email_verified_at" id="switch-one" class="switch" @if ($user->email_verified_at != null || $user->email_verified_at != '') checked @endif/>
                                <label for="switch-one" class="lbl-on">Verified</label>
                                <label for="switch-one" class="lbl-off">Unverified</label>
                            </div>
                        </div>
                        <div class="col-xl-3 col-md-6">
                            <label class="form-control-label h6 mt-1">Role</label>
                            <!-- <input type="checkbox" data-onstyle="primary" data-offstyle="warning" data-toggle="toggle" data-on="Admin" data-off="User" data-width="100%" name="role_id" @if ($user->role_id == 1) checked @endif> -->

                            <div class="button-switch">
                                <input type="checkbox" name="role_id" id="switch-two" class="switch" @if ($user->role_id == 1) checked @endif />
                                <label for="switch-two" class="lbl-on">Admin</label>
                                <label for="switch-two" class="lbl-off">User</label>
                            </div>
                        </div>
                        <div class="col-xl-3 col-md-6">
                            <label class="form-control-label h6 mt-1">{{ __('locale.Status') }} </label>
                            <!-- <input type="checkbox" data-onstyle="success" data-offstyle="danger" data-toggle="toggle" data-on="{{ __('locale.Active') }}" data-off="{{ __('locale.Banned') }}" data-width="100%" name="status" @if ($user->status == 1) checked @endif> -->

                            <div class="button-switch">
                                <input type="checkbox" name="status" id="switch-three" class="switch" @if ($user->status == 1) checked @endif />
                                <label for="switch-three" class="lbl-on">Active</label>
                                <label for="switch-three" class="lbl-off">Banned</label>
                            </div>
                        </div>
                    </div>
                ...
                <tr>
                    ...
                    <td data-label="{{ __('locale.Action') }}">
                        <div class="d-flex justify-content-start">
                            <a data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('locale.Add/Subtract Balance') }}" data-address="{{ $wallet->address }}" data-symbol="{{ $wallet->symbol }}" class="btn btn-icon btn-success btn-sm" onclick="$('#addSubModal').find('input[name=address]').val($(this).data('address'));
                                                            $('#addSubModal').find('input[name=symbol]').val($(this).data('symbol'));$('#addSubModal').modal('show');">
                                <i class="bi bi-cash"></i>
                            </a>
                            @if ($wallet->provider != 'funding')
                            <form class="ms-1" method="POST" action="{{ route('admin.wallet.regenerate') }}">
                                @csrf
                                <input type="hidden" id="user_id" name="user_id" value="{{ $user->id }}">
                                <input type="hidden" id="address" name="address" value="{{ $wallet->address }}">
                                <input type="hidden" id="type" name="type" value="{{ $wallet->type }}">
                                <button type="submit" class="btn btn-icon btn-warning btn-sm" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('locale.Regenerate Wallet') }}"><i class="bi bi-arrow-repeat"></i></button>
                            </form>
                            @endif
                        </div>
                    </td>
                ...
                <tr>
                    ...
                    <td data-label="{{ __('locale.Action') }}">
                        <form method="POST" action="{{ route('admin.wallet.create') }}">
                            ...
                            <input type="hidden" id="type" name="type" value="{{ $wallet_type }}">
...
<div class="col-xl-3 col-lg-5 col-md-5">
    <div class="card rounded-2 overflow-hidden shadow">
        <div class="card-body p-0">
            <div class="p-2 bg-white d-flex align-items-center">
                <div class="">
                    <img src="{{ getImage(imagePath()['profileImage']['path'] . '/' . $user->profile_photo_path, imagePath()['profileImage']['size']) }}" alt="{{ __('locale.Profile Image') }}" class="w-100">
                </div>
                <div class="mt-15 ps-2">
        ...
        <div class="card-body">
            ...
            <ul class="list-group">
                ...
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    {{ __('locale.Wallet Balance(in USD)') }}
                    <span class="h6 mt-1">{{ $user->balance }}</span>
                </li>
                ...
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    ...
                    <span class="float-end text-muted">{{ __($refer_by->firstname).' ' . __($refer_by->lastname)}}</span>
                </li>
                @endif
            </ul>
        </div>
    ...
    <div class="card rounded-2 overflow-hidden mt-2 shadow">
        <div class="card-body btn-padding">
            <h5 class="text-muted mb-0">{{ __('locale.User action') }}</h5>
            <a href="{{ route('admin.users.email.single', $user->id) }}" class="btn btn-danger mt-1 " style="width:150px;margin-right:30px;">
                {{ __('locale.Send Email') }}
            </a>

            <a href="{{ route('admin.users.referral.log', $user->id) }}" class="btn btn-info mt-1" style="width:150px;">
                {{ __('locale.Referral Log') }}
            </a>

            {{--<a href="{{ route('admin.users.commission.log', $user->id) }}" class="btn btn-warning mt-1">
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header border-bottom">
                <h4 class="modal-title">{{ __('locale.Add / Subtract Balance') }}</h4>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col col-md-12">
                            <!-- <input class="form-check-input" data-width="100%" data-size="lg" type="checkbox" data-toggle="toggle" data-onstyle="success" data-offstyle="danger" data-on="{{ __('locale.Add Balance') }}" data-off="{{ __('locale.Subtract Balance') }}" name="act" checked> -->

                            <div class="button-switch ms-auto me-auto">
                                <input type="checkbox" name="act" id="switch-four" class="switch" checked />
                                <label for="switch-four" class="lbl-on">{{ __('locale.Add Balance') }}</label>
                                <label for="switch-four" class="lbl-off">{{ __('locale.Subtract Balance') }}</label>
                            </div>
                        </div>
