    ...
    @if(request()->routeIs('admin.users.transactions'))
        <form action="" method="GET" class=" float-sm-right bg--white">
            <div class="input-group border">
                <input type="text" name="search" class="form-control border-0" placeholder="{{ __('locale.TRX / Username')}}" value="{{ $search ?? '' }}">
                    <button class="btn btn-primary waves-effect waves-float waves-light" type="submit"><i class="bi bi-search"></i></button>
            </div>
        </form>
    @else
