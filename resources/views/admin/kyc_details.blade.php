            ...
            <div class="card-innr">
                ...
                <ul class="data-details-list">
                    ...
                    <li>
                        ...
                        @if($kyc->user!=null)
                        <div class="data-details-des">{!! ($kyc->user->email) ? explode_user_for_demo($kyc->user->email, auth()->user()->type) : $space !!}</div>
                        @endif
                    </li>{{-- li --}}
                    <li>
                        <div class="data-details-head">Phone Number</div>
                        @if($kyc->user!=null)
                        <div class="data-details-des">{!! ($kyc->user->country_code.' '.$kyc->user->mobile) ? _x($kyc->user->country_code.' '.$kyc->user->mobile) : $space !!}</div>
                        @endif
                    </li>{{-- li --}}
                    <li>
                        <div class="data-details-head">Date of Birth</div>
                        @if($kyc->user!=null)
                        <div class="data-details-des">{!! ($kyc->user->dob) ? _date($kyc->user->dob, get_setting('site_date_format')) : $space !!}</div>
                        @endif
                    </li>{{-- li --}}
                    <li>
                    ...
                    <li>
                        ...
                        @if($kyc->user!=null)
                        <div class="data-details-des">{!! ($kyc->user->country) ? _x($kyc->user->country) : $space !!}</div>
                        @endif
                    </li>{{-- li --}}
                    <li>
                ...
                <ul class="data-details-list">
                    <li>
                        <div class="data-details-head me-2">
...
<div id="kyc_action" class="modal fade text-start 34" tabindex="-1" aria-hidden="true" role="dialog">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">{{ __('locale.KYC') }} Update Confirmation</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('admin.kyc.update') }}" method="POST" id="kyc_status_form" class="card-body p-2">
                @csrf
                <div class="input-group mb-1">
                    <label class="btn btn-primary" for="inputGroupSelect01">Status</label>
...
<div id="deleteModal" class="modal fade text-start" tabindex="-1" aria-hidden="true" role="dialog">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">{{ __('locale.KYC') }} Delete Confirmation</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
